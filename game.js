...
class ShootingStar {
  constructor(x, y) {
    ...
    for (let i = 0; i < n * 2; i++) {
      ...
      this.verts.push([Math.cos(ang) * this.radius * r, Math.sin(ang) * this.radius * r]);
    }
  }
  update(dt) {
  ...
  draw() {
    for (let i = this.history.length - 1; i >= 0; i--) {
      ...
      const a = (1 - i / this.history.length) * 0.5;
      ctx.strokeStyle = `rgba(255,255,255,${a.toFixed(2)})`;
      ctx.lineWidth   = 1.5;
      ctx.beginPath();
      ctx.arc(hx, hy, this.radius * (1 - i / this.history.length * 0.7), 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.save();
    ctx.translate(this.x, this.y);
...
class Ship {
  ...
  update(dt) {
    ...
    if (this.thrusting) {
      const T = THRUST * this.speedMultiplier;
      this.vx += Math.cos(this.angle) * T * dt;
      this.vy += Math.sin(this.angle) * T * dt;
    }
  ...
  draw() {
    ...
    // ponytail: ring estático sin pulso; sumar Math.sin(t*8)*0.3 al alpha si querés feedback al recibir impacto
    if (this.shielded) {
      ctx.beginPath();
...
class PowerUp {
  ...
  draw() {
    ...
    ctx.moveTo( 2, -7); ctx.lineTo(12,  0); ctx.lineTo( 2,  7);
    ctx.stroke();
    ctx.restore();
  }
}
...
function update(dt) {
  ...
  if (ship.invincible <= 0 && !ship.shielded) {
    ...
    for (const s of shootingStars) {
      if (!s.dead && !ship.dead && dist(ship, s) < ship.radius + s.radius) {
        killShip();
        break;
      }
    }
